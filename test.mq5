...
//|             Uses cryptocurrency.cv free API (200+ sources, unlim) |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   Print("══════════════════════════════════════════════════════════════");
   Print("  CRYPTO NEWS SENTIMENT EA - FULLY FREE (MQL5)");
...
//+------------------------------------------------------------------+
void ParseKeywords(string input_str, string &output[])
{
   string temp[];
   ushort separator = StringGetCharacter(",", 0);
   int count = StringSplit(input_str, separator, temp);
   ArrayResize(output, count);

   for(int i = 0; i < count; i++)
   {
      output[i] = StrTrim(StrToLower(temp[i]));
   }
}
...
void FetchNewsAndAnalyze()
{
   ...
   if(response == "")
   {
      Print(" Failed to fetch news. Using cached sentiment.");
      return;
   }
...
//+------------------------------------------------------------------+
//| FETCH NEWS FROM FREE API - CORRECTED VERSION                     |
//+------------------------------------------------------------------+
...
{
   string lowerHeadline = StrToLower(headline);
...
void ExecuteTrade(int signal)
{
   ...
   if(signal == 1) // BUY (Fear)
   {
      ...
      success = trade.Buy(lotSize, _Symbol, tick.ask, sl, tp, comment);
   }
   else if(signal == -1) // SELL (Greed)
   {
      ...
      success = trade.Sell(lotSize, _Symbol, tick.bid, sl, tp, comment);
   }
